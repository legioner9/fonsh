...
"
    ...
    if ! _rr &>${HOME}/.tmp; then
        echo "in fs= file://${HOME}.st.rc.d/.st.sh.d/_is_bashrc.sh , line=${LINENO}, ${FNN}() : : EXEC_FAIL : '_rr' : ${hint} :  flag=1" >&2
        flag=1
    fi
    ...
    if [[ -n "$(cat ${HOME}/.tmp)" ]]; then
        echo "exec _rr :" >&2
        cat ${HOME}/.tmp >&2
        echo "in fs= file://${HOME}.st.rc.d/.st.sh.d/_is_bashrc.sh , line=${LINENO}, ${FNN}() : : EXEC_TRUE : '[[ -n \$(cat ${HOME}/.tmp) ]] : ${hint} :  flag=1" >&2
        flag=1
    fi

    if [[ ${flag} -eq 1 ]]; then
        echo "in fs= file://${HOME}.st.rc.d/.st.sh.d/_is_bashrc.sh , line=${LINENO}, ${FNN}() return 1" >&2
        return 1
    fi
