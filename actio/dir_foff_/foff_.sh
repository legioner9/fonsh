...
filename="${FONSH_PATH}/actio/dir_foff_/foff_.sh"
...
exit_err_foff_() { #* $1 error message
local arg1="$1"
unset_autoargs_foff_
...
# >${FN_OUT_DIR}/com_foff_.res
# COMMAND &>>${FN_OUT_DIR}/com_foff_.res
# if [ $? -ne 0 ]; then
#     if [ 1 -eq ${verbose} ]; then
#         cat ${FN_OUT_DIR}/com_foff_.res
#     fi
#     #? runtime error allowed locally
#     error_allowed_locall_string="STRING"
#     if cat ${FN_OUT_DIR}/COMMAND_foff_.res | is_rl_siev_and ${error_allowed_locall_string}; then
#         continue
#     else
#         exit_err_foff_ "runtime error:: COMMAND ${addr}"
#     fi
# fi
#! amount args
